...
const mockFileTreeData = [
  {
    ...
    children: [
      ...
      { id: '1-2', name: 'libs', type: 'folder', children: [
...
function FileTreeItem({ node, level = 0, onSelect, basePath = '' }) {
  ...
  return (
    <div>
      <div
        style={{ ...indentStyle, cursor: node.type === 'folder' ? 'pointer' : 'default', fontWeight: node.type === 'folder' ? 'bold' : 'normal' }}
      ...
      {isOpen && node.children && node.children.map(child => (
        <FileTreeItem key={child.id} node={child} level={level + 1} onSelect={onSelect} basePath={currentPath} />
      ))}
    </div>
  );
}
...
export default function FileTreePanel({ onSelect }) {
  return (
    <div className="jamai-file-tree-panel" style={{ width: 220, borderRight: '1px solid #ddd', height: '100%', overflowY: 'auto', background: '#fafaff' }}>
